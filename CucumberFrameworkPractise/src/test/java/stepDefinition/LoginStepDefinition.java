...
public class LoginStepDefinition extends EnvironmentSetup {
	...
	public void Enter_valid_uesrname_and_valid_password(DataTable credentials) {
		try {
			...
			for (int i = 0; i < listOfcredentials.size(); i++) {
				System.out.println("Entering username : " + listOfcredentials.get(i).get("username")
						+ " Entering password : " + listOfcredentials.get(i).get("password"));
	...
	public void verifyTitleOfPostPage() throws Exception {
		...
		// Making it fail intentionally to generate a fail report and get the screenshot
		// attached.
	...
	public void verifyAllLinksUnderPostMenu() {
		...
		for (int i = 0; i < postsPage.listOfLinksUnderPostMenu.size(); i++) {
			...
			Assert.assertTrue("FAILED to verify all sub menus of Post Menu.",
					subMenuLinkText.contains(postsPage.listOfLinksUnderPostMenu.get(i).getText()));
		}
	}
	...
	public void clickLogoutAndVerifyUserRedirectedToLoginPage() {
		try {
			...
			Assert.assertTrue("FAILED to verify title of Login page after logout.",
					driver.getTitle().contains("Log In "));
		} catch (Exception e) {
			System.out.println(
					"Exception caught in clickLogoutAndVerifyUserRedirectedToLoginPage() method : " + e.getMessage());
			e.printStackTrace();
		}
	}
}
